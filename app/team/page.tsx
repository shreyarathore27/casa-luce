...
const Team = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-950 via-amber-900 to-stone-900 relative overflow-hidden">
      {/* Subtle Background Elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-amber-700/20 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-float"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-stone-700/20 rounded-full mix-blend-multiply filter blur-xl opacity-25 animate-float-delayed"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-amber-600/15 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse-slow"></div>
      </div>
      ...
      <div className="relative z-10 pt-20 pb-16 px-4">
        <div className="text-center">
          <h1 
            ...
            data-animate-key="header"
            className={`text-6xl md:text-7xl font-bold bg-gradient-to-r from-amber-200 via-stone-200 to-amber-300 bg-clip-text text-transparent mb-4 tracking-tight transform transition-all duration-1000 ease-out ${
          ...
          <div 
            ref={setElementRef('divider')}
            data-animate-key="divider"
            className={`w-32 h-1 bg-gradient-to-r from-amber-600 to-stone-600 mx-auto mb-6 rounded-full opacity-80 transform transition-all duration-800 ease-out ${
              visibleElements.has('divider') 
                ? 'scale-x-100 opacity-80' 
                : 'scale-x-0 opacity-0'
            }`}
            style={{
              transitionDelay: visibleElements.has('divider') ? '300ms' : '0ms'
            }}
          />
          
          <p 
            ref={setElementRef('subtitle')}
            data-animate-key="subtitle"
            className={`text-xl text-white max-w-3xl mx-auto font-light transform transition-all duration-800 ease-out ${
              visibleElements.has('subtitle') 
                ? 'translate-y-0 opacity-100' 
                : 'translate-y-8 opacity-0'
            }`}
            style={{
              transitionDelay: visibleElements.has('subtitle') ? '500ms' : '0ms'
            }}
          >
            The visionaries behind our success story
          </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
            {teamMembers.map((member, index) => (
              <div 
                key={member.id}
                ref={setElementRef(`card-${member.id}`)}
                data-animate-key={`card-${member.id}`}
                className={`group transform transition-all duration-1000 ease-out hover:scale-102 ${
                  visibleElements.has(`card-${member.id}`) 
                    ? 'translate-y-0 opacity-100 scale-100' 
                    : 'translate-y-16 opacity-0 scale-95'
                }`}
                style={{
                  transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 200}ms` : '0ms'
                }}
              >
                {/* Main Card */}
                <div className="relative bg-amber-900/30 backdrop-blur-xl rounded-3xl p-8 border border-amber-600/20 shadow-xl hover:shadow-amber-900/30 transition-all duration-400 hover:border-amber-500/30 hover:bg-amber-800/35">
                  ...
                  {/* Minimal Decorative Elements */}
                  <div className="absolute top-4 right-4 w-8 h-8 border border-amber-500/15 rounded-full animate-pulse-subtle"></div>
                  <div className="absolute bottom-4 left-4 w-6 h-6 border border-stone-400/20 rounded-full animate-pulse-subtle-delayed"></div>
                  ...
                  <div className="relative z-10 flex flex-col items-center text-center">
                    ...
                    {/* Image Container with Subtle Glow */}
                    <div 
                      className={`relative mb-8 transform transition-all duration-800 ease-out ${
                        visibleElements.has(`card-${member.id}`) 
                          ? 'translate-y-0 opacity-100 scale-100' 
                          : 'translate-y-8 opacity-0 scale-90'
                      }`}
                      style={{
                        transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 500}ms` : '0ms'
                      }}
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-amber-600/20 via-stone-500/15 to-amber-700/20 rounded-full blur-md opacity-40 animate-glow"></div>
                      <div className="relative w-56 h-56 rounded-full overflow-hidden border-2 border-amber-400/25 shadow-lg bg-stone-100">
                        <img
                          src={member.image}
                          alt={member.name}
                      ...
                      {/* Subtle Ring */}
                      <div className="absolute inset-0 w-56 h-56 rounded-full border border-amber-500/20 opacity-50 animate-rotate-slow"></div>
                    </div>

                    {/* Name with Refined Gradient */}
                    <h2 
                      className={`text-2xl md:text-3xl font-bold mb-3 bg-gradient-to-r from-amber-200 via-stone-100 to-amber-300 bg-clip-text text-transparent tracking-wide transform transition-all duration-700 ease-out ${
                        visibleElements.has(`card-${member.id}`) 
                          ? 'translate-x-0 opacity-100' 
                          : 'translate-x-8 opacity-0'
                      }`}
                      style={{
                        transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 700}ms` : '0ms'
                      }}
                    >
                      {member.name}
                    </h2>
                    ...
                    {/* Title */}
                    <h3 
                      className={`text-base font-medium text-white mb-6 px-5 py-2 bg-amber-800/30 rounded-full backdrop-blur-sm border border-amber-500/20 shadow-md transform transition-all duration-700 ease-out ${
                        visibleElements.has(`card-${member.id}`) 
                          ? 'translate-x-0 opacity-100 scale-100' 
                          : '-translate-x-8 opacity-0 scale-95'
                      }`}
                      style={{
                        transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 900}ms` : '0ms'
                      }}
                    >
                      {member.title}
                    </h3>
                    ...
                    {/* Bio */}
                    <p 
                      className={`text-sm text-white leading-relaxed mb-8 max-w-lg text-center font-light backdrop-blur-sm bg-amber-900/15 p-5 rounded-2xl border border-amber-500/15 transform transition-all duration-800 ease-out ${
                        visibleElements.has(`card-${member.id}`) 
                          ? 'translate-y-0 opacity-100' 
                          : 'translate-y-6 opacity-0'
                      }`}
                      style={{
                        transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 1100}ms` : '0ms'
                      }}
                    >
                      {member.bio}
                    </p>
                    ...
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`group/btn relative inline-flex items-center gap-3 px-6 py-3 bg-amber-800/40 backdrop-blur-sm rounded-full border border-amber-500/30 text-amber-100 font-medium transition-all duration-500 hover:bg-amber-700/50 hover:border-amber-400/40 hover:scale-102 hover:shadow-lg hover:text-amber-50 transform ${
                        visibleElements.has(`card-${member.id}`) 
                          ? 'translate-y-0 opacity-100 scale-100' 
                          : 'translate-y-8 opacity-0 scale-90'
                      }`}
                      style={{
                        transitionDelay: visibleElements.has(`card-${member.id}`) ? `${index * 300 + 1300}ms` : '0ms'
                      }}
                    >
                      <svg 
                        width="20" 
      ...
      <style jsx>{`
        ...
        .animate-pulse-subtle-delayed {
          animation: pulse-subtle-delayed 5s ease-in-out infinite;
