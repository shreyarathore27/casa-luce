  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="hidden lg:flex space-x-12">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`relative text-sm font-medium tracking-wider transition-all duration-300 hover:text-[#9e9268] group ${
                  pathname === link.href
                    ? 'text-[#9e9268]'
                    : 'text-white'
                }`}
              >
                {link.label}
                <span className="absolute -bottom-2 left-0 w-0 h-0.5 bg-[#9e9268] transition-all duration-300 group-hover:w-full"></span>
                {pathname === link.href && (
        ...
        >
          <div className="bg-black/90 backdrop-blur-lg rounded-2xl mt-2 p-6 space-y-4">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`block text-lg font-medium tracking-wide transition-colors duration-300 hover:text-[#9e9268] py-2 ${
                  pathname === link.href
                    ? 'text-[#9e9268]'
                    : 'text-white'
                }`}
